...
export class ModeOperations {
    ...
    deleteCurrentVariation() {
        ...
        if (siblings.length <= 1)
            return false; // 主分岐は消せない
        const idx = siblings.findIndex((n) => n.id === currentNode.id);
        if (idx <= 0)
            return false; // 主分岐(0番目)は消せない
    ...
    promoteCurrentToMain() {
        ...
        if (idx <= 0) {
            ...
            for (const s of siblings)
                s.isMainLine = s.id === currentNode.id;
            return;
        }
    ...
    buildPathFromRoot(nodeId) {
        const path = [];
        let current = this.findNodeById(nodeId);
        while (current) {
