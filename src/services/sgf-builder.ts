...
const clampCoord = (value: number): number => Math.max(0, value);

const encodeCoordinate = (value: number): string =>
  LETTERS.charAt(clampCoord(value));

const buildSetupProperty = (
  property: "AB" | "AW",
  positions: Position[]
): string => {
...
export const buildProblemSGF = (
  boardSize: number,
  black: Position[],
  white: Position[]
): string => {
  const header = `(;GM[1]FF[4]SZ[${boardSize}]`;
  return `${header}${buildSetupProperty("AB", black)}${buildSetupProperty(
    "AW",
    white
  )})`;
};

export const buildSolutionSGF = (
  problemSGF: string,
  moves: Move[],
  boardSize: number
): string => {
  ...
  const sequence = moves
    .map((move) => {
      ...
      return `;${color}[${encodeCoordinate(move.col)}${encodeCoordinate(
        move.row
      )}]`;
    })
    .join("");
