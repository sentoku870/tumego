// ============ AppFactory ============
// compositionRoot() をクラス化。テスト時は依存 (engine, parser) を
// register() で差し替え可能。compositionRoot() 関数も後方互換として残す。
import { GameState, UIElements } from '../types.js';
...
export interface AppContext {
  ...
  qrManager: QRManager;
  boardCapture: BoardCaptureService;
  dropdownManager: DropdownManager;
  eventBus: UIEventBus;
  controllers: {
    board: BoardInteractionController;
    toolbar: ToolbarController;
    feature: FeatureMenuController;
...
export class AppFactory {
  private engineOverride: GoEngine | null = null;
  private sgfParserOverride: SGFParser | null = null;

  /** テスト用: Engine を差し替え */
  registerEngine(engine: GoEngine): this {
    this.engineOverride = engine;
    return this;
  }

  /** テスト用: SGFParser を差し替え */
  registerSgfParser(parser: SGFParser): this {
    this.sgfParserOverride = parser;
    return this;
  }

  /** アプリケーションコンテキストを生成する */
  create(state: GameState, elements: UIElements): AppContext {
    const eventBus = new UIEventBus();

    const engine = this.engineOverride ?? new GoEngine();
    const sgfParser = this.sgfParserOverride ?? new SGFParser();
    ...
    const boardCapture = new BoardCaptureService(
      elements.svg,
      renderSnapshot,
      (msg: string) => renderer.showMessage(msg)
    );
    ...
    const file = new FileMenuController(
      dropdownManager,
      sgfService,
      renderer,
      qrManager,
 ...
 */
export function compositionRoot(state: GameState, elements: UIElements): AppContext {
