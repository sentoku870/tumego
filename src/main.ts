...
type BoardImageErrorCode = 'detect-failed' | 'not-go' | 'user-cancelled';

class BoardImageError extends Error {
  constructor(message: string, public code: BoardImageErrorCode = 'detect-failed') {
    super(message);
...
interface StoneClassification {
  black: Map<string, StoneInfo>;
  white: Map<string, StoneInfo>;
}
...
class BoardImageImporter {
  ...
  constructor(private uiController: UIController) {
    const canvas = document.getElementById('goban-canvas') as HTMLCanvasElement | null;
    if (!canvas) {
  ...
  private ensureFileInput(): HTMLInputElement {
    let input = document.getElementById('board-image-input') as HTMLInputElement | null;
    if (!input) {
  ...
  private async tryClipboardImage(): Promise<boolean> {
  ...
  private async processImageBlob(blob: Blob, source: 'clipboard' | 'file'): Promise<void> {
  ...
  private handleProcessingError(error: unknown, source: 'clipboard' | 'file'): void {
  ...
  private async drawBlobToCanvas(blob: Blob): Promise<ImageData> {
  ...
  private async loadCanvasSource(blob: Blob): Promise<{ element: CanvasImageSource; width: number; height: number; cleanup?: () => void; }> {
  ...
  private detectGrid(grayscale: Float32Array, width: number, height: number): GridDetectionResult {
  ...
  private detectLines(grayscale: Float32Array, width: number, height: number, orientation: 'vertical' | 'horizontal'): number[] {
  ...
  private extractLinePositions(scores: Float32Array): number[] {
    ...
    for (let i = 1; i < length - 1; i++) {
      ...
      if (value < dynamicThreshold) continue;
  ...
  private isLikelyGoBoard(detection: GridDetectionResult): boolean {
  ...
  private async resolveBoardSize(detection: GridDetectionResult): Promise<number | null> {
  ...
  private adjustBoardSize(detected: number): number {
  ...
  private promptBoardSize(info: { detected: number; adjusted: number; confidence: number; }): Promise<number | null> {
  ...
  private classifyStones(
    ...
    const black = new Map<string, StoneInfo>();
    const white = new Map<string, StoneInfo>();
    ...
    for (let i = 0; i < detection.verticalLines.length; i++) {
      ...
      for (let j = 0; j < detection.horizontalLines.length; j++) {
        ...
        if (!statsInner || !statsOuter) continue;
        ...
        if (mappedCol < 0 || mappedCol >= boardSize || mappedRow < 0 || mappedRow >= boardSize) continue;
  ...
  private mapIndex(index: number, detectedCount: number, targetSize: number): number {
    if (detectedCount <= 1 || targetSize <= 1) return 0;
  ...
  private generateSGF(boardSize: number, stones: StoneClassification): string {
  ...
  private composeMessage(boardSize: number, stones: StoneClassification, detection: GridDetectionResult, source: 'clipboard' | 'file'): string {
  ...
  private smoothArray(values: Float32Array, radius: number): Float32Array {
  ...
  private averageSpacing(lines: number[]): number {
  ...
  private spacingUniformity(lines: number[]): number {
